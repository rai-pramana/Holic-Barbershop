...
<nav class="fixed top-0 left-0 right-0 z-50 bg-gray-950/80 backdrop-blur-xl border-b border-white/5">
    <div class="max-w-7xl mx-auto px-6 py-4 flex justify-between items-center">
        <div class="flex items-center gap-2">
            <div class="w-9 h-9 rounded-xl bg-gradient-to-br from-gray-900 to-slate-800 flex items-center justify-center glow">
                <svg class="w-5 h-5 text-white" fill="currentColor" viewBox="0 0 20 20">
                    <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"/>
                </svg>
            </div>
            <span class="font-bold text-lg">HOLIC <span class="text-gray-600">Barbershop</span></span>
        </div>
        <div class="flex items-center gap-4">
            @auth
                @if(auth()->user()->isAdmin())
                    <a href="{{ route('admin.dashboard') }}" class="text-sm font-medium bg-gradient-to-r from-gray-900 to-slate-800 text-white px-5 py-2.5 rounded-xl hover:opacity-90 transition-opacity">
                        Admin Panel
                    </a>
                @elseif(auth()->user()->isBarber())
                    <a href="{{ route('barber.dashboard') }}" class="text-sm font-medium bg-gradient-to-r from-gray-900 to-slate-800 text-white px-5 py-2.5 rounded-xl hover:opacity-90 transition-opacity">
                        Barber Panel
                    </a>
                @else
                    <a href="{{ route('customer.dashboard') }}" class="text-sm font-medium bg-gradient-to-r from-gray-900 to-slate-800 text-white px-5 py-2.5 rounded-xl hover:opacity-90 transition-opacity">
                        Dashboard Saya
                    </a>
                @endif
            @else
                <a href="{{ route('login') }}" class="text-sm text-gray-400 hover:text-white transition-colors font-medium">Masuk</a>
                <a href="{{ route('register') }}" class="text-sm font-semibold bg-gradient-to-r from-gray-900 to-slate-800 text-white px-5 py-2.5 rounded-xl hover:opacity-90 transition-opacity shadow-lg shadow-gray-900/20">
                    Daftar Gratis
                </a>
            @endauth
        </div>
                ...
                <div class="flex flex-col sm:flex-row gap-4">
                    @auth
                        <a href="{{ auth()->user()->isCustomer() ? route('customer.dashboard') : (auth()->user()->isAdmin() ? route('admin.dashboard') : route('barber.dashboard')) }}"
                           class="inline-flex items-center justify-center gap-2 bg-gradient-to-r from-gray-900 to-slate-800 text-white font-semibold px-8 py-4 rounded-2xl hover:opacity-90 transition-opacity shadow-xl shadow-gray-900/25 text-center">
    ...
    <div class="max-w-5xl mx-auto px-6">
        ...
        <div class="grid md:grid-cols-4 gap-6">
            ...
            <div class="relative text-center">
                <div class="w-16 h-16 rounded-2xl bg-gradient-to-br from-gray-900 to-slate-800 flex items-center justify-center mx-auto mb-4 shadow-lg shadow-gray-900/20">
...
<section class="py-24 bg-gray-950">
    <div class="max-w-3xl mx-auto px-6 text-center">
        <div class="bg-gradient-to-br from-gray-700/20 to-slate-600/10 border border-gray-500/20 rounded-3xl p-12">
            ...
            @guest
            <a href="{{ route('register') }}"
               class="inline-flex items-center gap-2 bg-gradient-to-r from-gray-900 to-slate-800 text-white font-bold px-10 py-4 rounded-2xl hover:opacity-90 transition-opacity shadow-xl shadow-gray-900/25 text-lg">
