...
<div class="space-y-6">
    ...
    <div class="bg-white rounded-2xl border border-gray-100 shadow-sm">
        <div class="p-6 border-b border-gray-100 flex justify-between items-center">
            ...
            <a href="{{ route('admin.queues.index') }}" class="text-sm text-pink-600 hover:text-pink-700 font-medium">
                Lihat Semua →
            </a>
        </div>
            ...
            <table class="w-full">
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    <tr class="hover:bg-gray-50/50 transition-colors">
                        ...
                        <td class="px-6 py-4">
                            <p class="font-medium text-gray-900 text-sm">{{ $queue->customer->name }}</p>
                            <p class="text-xs text-gray-500">{{ $queue->branch->name }}</p>
                        </td>
                        <td class="px-6 py-4 text-sm text-gray-700">{{ $queue->barber?->user?->name ?? '—' }}</td>
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <a href="{{ $link['href'] }}"
           class="bg-white border border-gray-200 rounded-2xl p-4 flex items-center gap-3 hover:border-pink-200 hover:shadow-md transition-all group">
            ...
            <span class="text-sm font-semibold text-gray-700 group-hover:text-pink-600 transition-colors">{{ $link['label'] }}</span>
        </a>
        @endforeach
    </div>
</div>
