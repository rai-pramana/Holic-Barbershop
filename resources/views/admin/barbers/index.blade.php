    ...
    <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
        ...
        <div class="flex gap-2 mt-4">
            <a href="{{ route('admin.barbers.show', $barber) }}"
               title="Lihat Detail"
               class="flex-1 flex items-center justify-center gap-1.5 text-xs font-semibold bg-gray-100 text-gray-700 py-2 rounded-xl hover:bg-gray-200 border border-gray-200 active:scale-95 transition-all">
                <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg>
                Lihat
            </a>
            <a href="{{ route('admin.barbers.edit', $barber) }}"
               title="Edit"
               class="flex-1 flex items-center justify-center gap-1.5 text-xs font-semibold bg-white text-gray-900 py-2 rounded-xl hover:bg-gray-100 border border-gray-200 active:scale-95 transition-all">
                <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
                Edit
            </a>
            <form method="POST" action="{{ route('admin.barbers.destroy', $barber) }}"
                  onsubmit="return confirm('Hapus barber {{ $barber->name }}? Tindakan ini tidak dapat dibatalkan.')">
                @csrf @method('DELETE')
                <button type="submit" title="Hapus"
                        class="flex items-center gap-1.5 text-xs font-semibold bg-gray-100 text-gray-600 py-2 px-3 rounded-xl hover:bg-gray-200 border border-gray-200 active:scale-95 transition-all h-full">
