    ...
    <div class="flex flex-wrap gap-3 items-center">
        ...
        <div class="flex gap-2 flex-wrap">
            ...
            <button type="button" onclick="setPreset('{{ $key }}')"
                    ...
                    class="px-4 py-2 rounded-xl text-xs font-semibold border transition-all preset-btn
                           {{ $preset === $key ? 'bg-gradient-to-r from-gray-900 to-slate-800 text-white border-transparent shadow-sm' : 'border-gray-200 text-gray-600 hover:border-gray-300 hover:text-gray-900' }}">
                {{ $label }}
            </button>
            @endforeach
        </div>
        ...
        <div class="flex items-center gap-2 ml-auto">
            ...
            <select name="branch_id" onchange="this.form.submit()"
                    class="border border-gray-200 rounded-xl px-3 py-2 text-sm text-gray-700 bg-gray-50 outline-none focus:border-gray-300 cursor-pointer">
...
<div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4 mb-6">
    ...
    <a href="{{ $kpi['link'] }}"
       class="bg-white rounded-2xl border border-gray-100 shadow-sm p-4 flex flex-col gap-2 hover:border-gray-200 hover:shadow-md transition-all group cursor-pointer">
        ...
        <div>
            <p class="text-2xl font-black text-gray-900 group-hover:text-gray-900 transition-colors">{{ $kpi['value'] }}</p>
            <p class="text-xs font-semibold text-gray-700 leading-tight flex items-center gap-1">
                {{ $kpi['label'] }}
                <svg class="w-3 h-3 text-gray-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14"/></svg>
            </p>
    ...
    <div class="flex items-center justify-between mb-4">
        <div>
            ...
            <p class="text-xs text-gray-400 mt-0.5">
                Jam tersibuk:
                <span class="text-gray-900 font-semibold">{{ str_pad($peakHour, 2, '0', STR_PAD_LEFT) }}:00 – {{ str_pad($peakHour+1, 2, '0', STR_PAD_LEFT) }}:00</span>
            ...
            <div class="px-6 py-4">
                <div class="flex items-center justify-between mb-2">
                    ...
                    <div class="text-right">
                        ...
                        <p class="text-xs font-semibold {{ $b['rate'] >= 80 ? 'text-emerald-600' : ($b['rate'] >= 50 ? 'text-gray-600' : 'text-rose-500') }}">
                            {{ $b['rate'] }}% selesai
                        </p>
                    </div>
                </div>
        ...
        <div class="divide-y divide-gray-50">
            ...
            <div class="px-6 py-4">
                ...
                <div class="h-2 bg-gray-100 rounded-full overflow-hidden">
                    <div class="h-full bg-gradient-to-r from-gray-800 to-slate-700 rounded-full transition-all"
