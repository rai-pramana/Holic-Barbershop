<?php $__env->startSection('title', 'Status Antrean #' . $queue->queue_number); ?>
...
<div class="max-w-lg mx-auto">
    ...
    <div class="bg-white rounded-3xl border border-gray-100 shadow-lg overflow-hidden mb-5" id="status-card">
        ...
        <div class="relative overflow-hidden text-white text-center
            <?php if($queue->status === 'called'): ?>    bg-gradient-to-br from-purple-500 to-indigo-600
            <?php elseif($queue->status === 'active'): ?>  bg-gradient-to-br from-blue-500 to-cyan-600
            <?php elseif($queue->status === 'pending'): ?> bg-gradient-to-br from-amber-400 to-orange-500
            ...
            <div class="relative z-10">
                <p class="text-white/70 text-xs font-semibold uppercase tracking-widest mb-1"><?php echo e($queue->branch->name); ?></p>
                ...
                ?>
                <div class="inline-flex items-center gap-2 bg-white/20 backdrop-blur-sm border border-white/30 rounded-full px-4 py-2 text-sm font-semibold <?php echo e($cfg['animate'] ? 'animate-pulse' : ''); ?>">
                    <svg class="w-4 h-4 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="<?php echo e($cfg['icon']); ?>"/></svg>
        ...
        <div class="p-5 md:p-6 space-y-5">
            ...
            <div class="grid grid-cols-3 gap-3">
                <div class="bg-gray-50 rounded-2xl p-4 col-span-1">
                    ...
                    <p class="font-bold text-gray-900 text-sm" id="queue-position">
                        <?php if($queue->isActive_or_Pending()): ?>
                            ke-<?php echo e($queue->position_in_queue); ?>
                ...
                <div class="bg-gray-50 rounded-2xl p-4 col-span-1">
                    ...
                    <p class="font-bold text-gray-900 text-sm" id="queues-ahead">
                        <?php if($queue->isActive_or_Pending()): ?>
                            <?php echo e($queuesAhead + $pendingAhead > 0 ? ($queuesAhead + $pendingAhead).' org' : 'Hampir!'); ?>
                ...
                <div class="bg-gray-50 rounded-2xl p-4 col-span-1">
                    ...
                    <p class="font-bold text-gray-900 text-sm" id="wait-time">
                        <?php if($queue->isActive_or_Pending() && $waitMinutes > 0): ?>
                            ~<?php echo e($waitMinutes); ?>m
                        <?php elseif($queue->isActive_or_Pending()): ?>
                            Segera!
                        <?php else: ?> —
                        <?php endif; ?>
                    </p>
                ...
                <div class="bg-gray-50 rounded-2xl p-4 col-span-1">
                    ...
                    <p class="font-bold text-gray-900 text-sm"><?php echo e($queue->barber?->name ?? '—'); ?></p>
                </div>
                <div class="bg-gray-50 rounded-2xl p-4 col-span-2">
                    ...
                    <p class="font-bold text-gray-900 text-sm leading-tight"><?php echo e($queue->service->name); ?> <span class="text-xs font-normal text-gray-400">(<?php echo e($queue->service->duration_minutes); ?>m)</span></p>
                </div>
            </div>
            ...
            <div class="bg-gradient-to-br from-blue-50 to-indigo-50 border border-blue-200 rounded-2xl p-5">
                ...
                <div class="space-y-3">
                    <div class="flex items-start gap-3">
                        ...
                        <div>
                            ...
                            <p class="text-xs text-blue-600"><?php echo e($queue->branch->name); ?></p>
                        </div>
                    </div>
                    <div class="flex items-start gap-3">
                ...
                <?php if($queue->expired_at): ?>
                <div class="mt-4 text-center">
                    <p class="text-xs text-blue-700 font-semibold">
                        ⏰ Berlaku s.d. <span class="local-time" data-utc="<?php echo e($queue->expired_at->toISOString()); ?>"><?php echo e($queue->expired_at->format('H:i')); ?></span> WITA
                    </p>
                </div>
                <?php endif; ?>

                <p class="text-center text-blue-800 font-mono font-black text-2xl tracking-widest mt-4"><?php echo e($queue->queue_number); ?></p>
            </div>
            <?php endif; ?>


            
            <?php if($queue->isCalled()): ?>
            <div class="bg-purple-50 border-2 border-purple-300 rounded-2xl p-4 animate-pulse">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p class="text-purple-700 text-xs">Segera menuju kursi barber <strong><?php echo e($queue->barber->name); ?></strong></p>
                    </div>
                </div>
            </div>
            <?php endif; ?>
            ...
            <div class="border-t border-gray-100 pt-4">
                ...
                <div class="space-y-2.5">
                    ...
                    ?>
                    <?php $__currentLoopData = array_filter($timelineItems); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <div class="flex items-center gap-3">
                        <div class="w-2 h-2 rounded-full <?php echo e($item['dot']); ?> flex-shrink-0"></div>
                        <span class="text-sm text-gray-500 flex-1"><?php echo e($item['label']); ?></span>
                        <span class="text-sm font-semibold text-gray-800"><?php echo e($item['time']->format('H:i')); ?></span>
                    </div>
                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                    <?php if($queue->isPending() && $queue->expired_at): ?>
                    <div class="flex items-center gap-3">
                        <div class="w-2 h-2 rounded-full bg-red-300 flex-shrink-0"></div>
                        <span class="text-sm text-gray-400 flex-1">Kedaluwarsa pada</span>
                        <span class="text-sm font-semibold text-red-500"><?php echo e($queue->expired_at->format('H:i')); ?></span>
                    </div>
                    <?php endif; ?>
                </div>
            </div>
    ...
    <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-4">
        ...
        <p class="text-sm text-gray-700"><?php echo e($queue->notes); ?></p>
    </div>
    <?php endif; ?>
</div>
...
const UNSUBSCRIBE_URL  = "<?php echo e(route('customer.push.unsubscribe')); ?>";
...
// ─── Live Polling ──────────────────────────────────────────────────────────────
const pollUrl  = "<?php echo e(route('customer.queue.poll', $queue)); ?>";
let prevStatus = "<?php echo e($queue->status); ?>";
            ...
            if (msgData) {
                ...
                if (Notification.permission === 'granted') {
                    new Notification(msgData.title, {
                        ...
                        tag: 'queue-status-<?php echo e($queue->id); ?>',
                        renotify: true,
                    });
                }
            }
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\raipr\Documents\Code\holic-barbershop\resources\views\customer\queue\status.blade.php ENDPATH**/ ?>